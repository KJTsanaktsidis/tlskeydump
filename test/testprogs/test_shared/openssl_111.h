...
private:
  ...
  std::unique_ptr<SSL_CTX, decltype(&SSL_CTX_free)> _ssl_ctx = std::unique_ptr<SSL_CTX, decltype(&SSL_CTX_free)>(nullptr, SSL_CTX_free);
  std::unique_ptr<BIO, decltype(&BIO_free_all)> _bio = std::unique_ptr<BIO, decltype(&BIO_free_all)>(nullptr, BIO_free_all);
